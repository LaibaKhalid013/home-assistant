...
ENTITY_ID3 = f"{COVER_DOMAIN}.{slugify(DEVICE3.name)}"
ENTITY_ID3_2 = f"{COVER_DOMAIN}.{slugify(DEVICE3.name)}_2"
...
async def test_cover(
    ...
    entity_id: str,
    cover_id: int,
    position_open: list[int] | int,
    position_close: list[int] | int,
    direction_open: list[ShutterDirection] | ShutterDirection,
    direction_close: list[ShutterDirection] | ShutterDirection,
    direction_stop: list[ShutterDirection] | ShutterDirection,
) -> None:
    """Test cover services."""
